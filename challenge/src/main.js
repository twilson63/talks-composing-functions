...
try {
  ...
  async function run() {
    ...
    const medicationRequests = filter(propEq('resourceType', 'MedicationRequest'), resources)   
    //TODO
    ...
    const rxnorms = map(find(propEq('system', 'http://www.nlm.nih.gov/research/umls/rxnorm')), codings)
    const results = map(only(['code', 'display']), rxnorms)
    console.log(results)
    
    const results2 = compose(
     map(only(['code', 'display'])),
     map(find(propEq('system', 'http://www.nlm.nih.gov/research/umls/rxnorm'))),
     pluck('coding'),
     pluck('medicationCodeableConcept'),
